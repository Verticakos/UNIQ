				...
				if hum and (not state.aimbotIgnoreDead or hum.Health>0) then
					...
					if aimPos then
						...
						if meters>=state.aimMinDist and meters<=state.aimMaxDist then
							...
							if on then local visible=not state.aimbotVisibleOnly local hit=visible and nil or workspace:Raycast(Camera.CFrame.Position,aimPos-Camera.CFrame.Position) if visible or not hit or hit.Instance:IsDescendantOf(plr.Character) then local d=(Vector2.new(sp.X,sp.Y)-center).Magnitude if d<bestDist then bestDist=d best=aimPos end end end
						end
					end
				end
			end
...
local SKT=1 local BoxThick=1 local TracerThick=1 local FillOpacity=40 local JO=1
...
renderConn=RunService.RenderStepped:Connect(function()
	...
	for _,p in ipairs(Players:GetPlayers()) do
		...
		if NameE or DisplayTagsE then local tagText=DisplayTagsE and p.DisplayName or p.Name if not NLabels[p] then mkLbl(p,NLabels,{size=UDim2.new(0, 140, 0, 20),off=Vector3.new(0,4.3,0),text=tagText}) end local lb=NLabels[p] if lb and root then local g=lb.Parent if g then g.StudsOffsetWorldSpace=Vector3.new(0,4.3+math.clamp(studs/120,0,1.2),0) end local isFr=state.friends[p.Name] or state.friends[p.DisplayName] or state.friends[p.DisplayName] or state.friends[tostring(p.UserId)] lb.TextColor3=isFr and Color3.fromRGB(0,166,255) or Color3.new(1,1,1) lb.TextSize=math.clamp(18-studs/20,10,18) lb.Text=tagText end else rmNm(p) end
...
local function shutdown()
	...
	-- [MODIFIED] Ensure active ScreenGuis are completely destroyed on shutdown/unload
...
-- UI THEME
local T={Window=Color3.fromRGB(18,18,18),Card=Color3.fromRGB(30,30,30),SliderBg=Color3.fromRGB(39,39,39),Rail=Color3.fromRGB(14,14,14),Track=Color3.fromRGB(43,43,43),Stroke=Color3.fromRGB(36,36,36),Accent=Color3.fromRGB(0,156,255),AccentLight=Color3.fromRGB(0,156,255),Text=Color3.fromRGB(235,235,235),ValTxt=Color3.fromRGB(200,200,200),Muted=Color3.fromRGB(155,155,155),KbGrey=Color3.fromRGB(70,70,70)}
...
local FULL=UDim2.new(0,800,0,548)
...
local Rail=nn("Frame",{Size=UDim2.new(0,67,1,0),BackgroundColor3=T.Rail,BorderSizePixel=0,Parent=Win}) corner(Rail,6)
local Logo=nn("ImageLabel",{Size=UDim2.new(0,79,0,79),Position=UDim2.new(0.5,-39.5,0,-5),BackgroundTransparency=1,ImageColor3=Color3.new(1,1,1),ScaleType=Enum.ScaleType.Fit,ZIndex=2,Parent=Rail}) setIcon(Logo,"115384685356525")
local RailBox=nn("Frame",{Size=UDim2.new(1,0,1,-100),Position=UDim2.new(0,0,0,89),BackgroundTransparency=1,Parent=Rail}) local rl=list(RailBox,13) rl.HorizontalAlignment=Enum.HorizontalAlignment.Center

local Header=nn("Frame",{Size=UDim2.new(1,-67,0,60),Position=UDim2.new(0,67,0,0),BackgroundTransparency=1,Parent=Win})
...
local MinBtn=nn("ImageButton",{Size=UDim2.new(0,20,0,20),Position=UDim2.new(1,-38,0.5,-14),BackgroundTransparency=1,ImageColor3=T.Muted,ScaleType=Enum.ScaleType.Fit,AutoButtonColor=false,Parent=Header}) setIcon(MinBtn,"83381966246889")
local Gear=nn("ImageButton",{Size=UDim2.new(0,20,0,20),Position=UDim2.new(1,-68,0.5,-14),BackgroundTransparency=1,ImageColor3=T.Muted,ScaleType=Enum.ScaleType.Fit,AutoButtonColor=false,Parent=Header}) setIcon(Gear,"118523834089694")
...
Gear.MouseEnter:Connect(function() tw(Gear,0.22,{Rotation=90}) end)
Gear.MouseLeave:Connect(function() tw(Gear,0.22,{Rotation=0}) end)
Gear.MouseButton1Click:Connect(function()
	tw(Gear,0.28,{Rotation=450})
	task.delay(0.29,function() if Gear.Parent then Gear.Rotation=90 end end)
end)
...
local minimized=false MinBtn.MouseButton1Click:Connect(function()
	...
	tw(MinBtn,0.25,{Rotation=minimized and 180 or 0})
	tw(Win,0.35,{Size=minimized and UDim2.new(0,300,0,58) or FULL})
	if not minimized then task.delay(0.35,function() if not minimized and content then content.Visible=true end end) end
end)
local Content=nn("Frame",{Name="Content",Size=UDim2.new(0,655,0,414),Position=UDim2.new(0,96,0,115),BackgroundTransparency=1,Parent=Win})
...
local function CreateTab(name,iconId,order)
	...
	local glow=nn("ImageLabel",{Size=UDim2.new(0,32,0,32),Position=UDim2.new(0.5,-16,0.5,-16),BackgroundTransparency=1,ImageColor3=T.Accent,ImageTransparency=1,ScaleType=Enum.ScaleType.Fit,ZIndex=1,Parent=btn}) setIcon(glow,iconId)
	local icon=nn("ImageLabel",{Size=UDim2.new(0,21,0,21),Position=UDim2.new(0.5,-10.5,0.5,-10.5),BackgroundTransparency=1,ImageColor3=T.Muted,ScaleType=Enum.ScaleType.Fit,ZIndex=2,Parent=btn}) setIcon(icon,iconId)
...
local sc=nn("ScrollingFrame",{Size=UDim2.new(1,-10,1,-46),Position=UDim2.new(0,10,0,40),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=2,ScrollBarImageColor3=T.Accent,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,Parent=card})
		list(sc,4)
		return sc
	end
...
Value=function(parent,text,min,max,default,dec,suffix,fn)
	local wrap=nn("Frame",{Size=UDim2.new(1,-4,0,48),BackgroundTransparency=1,Parent=parent})
	local panel=nn("Frame",{Size=UDim2.new(1,0,0,28),BackgroundColor3=T.SliderBg,BorderSizePixel=0,Parent=wrap}) corner(panel,2)
	nn("TextLabel",{Text=text,Font=Fn,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,12,0,0),Size=UDim2.new(.62,0,1,0),Parent=panel})
	local number=nn("TextLabel",{Text=string.format("%."..dec.."f",default)..(suffix or ""),Font=Fn,TextSize=13,TextColor3=T.ValTxt,TextXAlignment=Enum.TextXAlignment.Right,BackgroundTransparency=1,Position=UDim2.new(.38,0,0,0),Size=UDim2.new(.62,-12,1,0),Parent=panel})
	...
	local fill=nn("Frame",{Size=UDim2.new(math.clamp((default-min)/(max-min),0,1),0,1,0),BackgroundColor3=T.Accent,BorderSizePixel=0,Parent=rail})
	local hit=nn("TextButton",{Size=UDim2.new(1,0,0,18),Position=UDim2.new(0,0,0,22),BackgroundTransparency=1,Text="",Parent=wrap}) local dragging=false
	local function setFromX(x) local a=math.clamp((x-rail.AbsolutePosition.X)/rail.AbsoluteSize.X,0,1) local raw=min+(max-min)*a local value=(dec==0) and math.floor(raw+.5) or tonumber(string.format("%."..dec.."f",raw)) fill.Size=UDim2.new(a,0,1,0) number.Text=string.format("%."..dec.."f",value)..(suffix or "") if fn then fn(value) end end
...
-- Dropdown variant used by the Players tab: selections can remain open.
_G.UniqBaseDropdown=Dropdown
Dropdown=function(parent,text,source,default,fn,keepOpen)
	if not keepOpen then return _G.UniqBaseDropdown(parent,text,source,default,fn) end
	local wrap=nn("Frame",{Size=UDim2.new(1,-4,0,46),BackgroundTransparency=1,ClipsDescendants=true,Parent=parent})
	local current=default or "None"
	nn("TextLabel",{Text=text,Font=Fn,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,0,0,0),Size=UDim2.new(0.55,0,0,30),Parent=wrap})
	local value=nn("TextLabel",{Text=current,Font=Fn,TextSize=13,TextColor3=T.ValTxt,TextXAlignment=Enum.TextXAlignment.Right,BackgroundTransparency=1,Position=UDim2.new(0.45,-16,0,0),Size=UDim2.new(0.55,0,0,30),Parent=wrap})
	nn("Frame",{Size=UDim2.new(1,0,0,2),Position=UDim2.new(0,0,0,30),BackgroundColor3=T.Accent,BorderSizePixel=0,Parent=wrap})
	local options=nn("Frame",{Position=UDim2.new(0,0,0,40),Size=UDim2.new(1,0,0,0),BackgroundColor3=T.SliderBg,BorderSizePixel=0,ClipsDescendants=true,Parent=wrap}) corner(options,6) local layout=list(options,2) layout.HorizontalAlignment=Enum.HorizontalAlignment.Center nn("UIPadding",{PaddingTop=UDim.new(0,3),Parent=options})
	local function build()
		for _,child in pairs(options:GetChildren()) do if child:IsA("TextButton") then child:Destroy() end end
		local items=(type(source)=="function") and source() or source if not items or #items==0 then items={"(none)"} end
		for _,item in ipairs(items) do local plr=game:GetService("Players"):FindFirstChild(item) local isFr=state.friends[item] or (plr and (state.friends[plr.Name] or state.friends[plr.DisplayName])) local b=nn("TextButton",{Size=UDim2.new(1,-6,0,24),BackgroundColor3=T.Accent,BackgroundTransparency=(item==current) and .8 or 1,AutoButtonColor=false,Text=item,Font=Fn,TextSize=12,TextColor3=isFr and Color3.fromRGB(0,166,255) or T.Text,Parent=options}) corner(b,5) b.MouseButton1Click:Connect(function() if item~="(none)" then current=item if fn then fn(item) end build() end end) end
		value.Text=tostring(items[1]=="(none)" and 0 or #items)
		return #items
	end
	local function refreshList() if wrap.Parent then local count=build() options.Size=UDim2.new(1,0,0,count*26+6) wrap.Size=UDim2.new(1,-4,0,52+count*26) end end
...
-- BUILD TABS
do
Combat  = CreateTab("Combat",  "113941213386286", 1)
Player  = CreateTab("Player",  "136841993281886", 2)
Visuals = CreateTab("Visuals", "72545313930928",  3)
Online  = CreateTab("Online",  "115957001084004", 4)
Misc    = CreateTab("Misc",    "82749417460131",  5)
...
Condition(cL,"Ignore Friend",false,function(v) state.aimIgnoreFriend=v end)
local cR=Combat.Right("Customization")
Dropdown(cR,"Aim Method",{"Camera","MouseMoveRel","Silent"},"MouseMoveRel",function(v) state.aimMethod=v end)
...
Value(vR,"Box Thickness",1,5,1,1,"",function(v) SH["Box Thickness"](v) end)
...
-- Optional final argument keeps a dropdown open after choosing an option.
Dropdown=function(parent,text,source,default,fn,keepOpen)
	local wrap=nn("Frame",{Size=UDim2.new(1,-4,0,46),BackgroundTransparency=1,ClipsDescendants=true,Parent=parent})
	local current=default or "None"
	nn("TextLabel",{Text=text,Font=Fn,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,0,0,0),Size=UDim2.new(0.55,0,0,30),Parent=wrap})
	local valueLabel=nn("TextLabel",{Text=current,Font=Fn,TextSize=13,TextColor3=T.ValTxt,TextXAlignment=Enum.TextXAlignment.Right,BackgroundTransparency=1,Position=UDim2.new(0.45,-16,0,0),Size=UDim2.new(0.55,0,0,30),Parent=wrap})
	local chevron=nn("TextLabel",{Text="v",Font=FB,TextSize=12,TextColor3=T.Muted,BackgroundTransparency=1,Position=UDim2.new(1,-14,0,0),Size=UDim2.new(0,12,0,30),Parent=wrap})
	nn("Frame",{Size=UDim2.new(1,0,0,2),Position=UDim2.new(0,0,0,30),BackgroundColor3=T.Accent,BorderSizePixel=0,Parent=wrap})
	local optionsFrame=nn("Frame",{Position=UDim2.new(0,0,0,40),Size=UDim2.new(1,0,0,0),BackgroundColor3=T.SliderBg,BorderSizePixel=0,ClipsDescendants=true,Parent=wrap}) corner(optionsFrame,6) local ol=list(optionsFrame,2) ol.HorizontalAlignment=Enum.HorizontalAlignment.Center nn("UIPadding",{PaddingTop=UDim.new(0,3),Parent=optionsFrame})
	local hit=nn("TextButton",{Size=UDim2.new(1,0,0,40),BackgroundTransparency=1,Text="",AutoButtonColor=false,ZIndex=2,Parent=wrap})
	local open=false
	local function close() open=false tw(chevron,0.15,{Rotation=0}) tw(optionsFrame,0.2,{Size=UDim2.new(1,0,0,0)}) tw(wrap,0.2,{Size=UDim2.new(1,-4,0,46)}) end
	local function build()
		for _,child in pairs(optionsFrame:GetChildren()) do if child:IsA("TextButton") then child:Destroy() end end
		local options=(type(source)=="function") and source() or source if not options or #options==0 then options={"(none)"} end
		for _,option in ipairs(options) do
			local button=nn("TextButton",{Size=UDim2.new(1,-6,0,24),BackgroundColor3=T.Accent,BackgroundTransparency=(option==current) and 0.8 or 1,AutoButtonColor=false,Text=option,Font=Fn,TextSize=12,TextColor3=T.Text,Parent=optionsFrame}) corner(button,5)
			button.MouseButton1Click:Connect(function()
				if option=="(none)" then return end
				current=option valueLabel.Text=option if fn then fn(option) end
				if keepOpen then build() else close() end
			end)
		end
		return #options
	end
	hit.MouseButton1Click:Connect(function() open=not open if open then local count=build() local height=count*26+6 tw(chevron,0.15,{Rotation=180}) tw(optionsFrame,0.2,{Size=UDim2.new(1,0,0,height)}) tw(wrap,0.2,{Size=UDim2.new(1,-4,0,46+height)}) else close() end end)
