...
aimbotConn=RunService.RenderStepped:Connect(function()
	...
	local fovR=state.aimbotFov local best,bestDist=nil,fovR
...
local function corner(p,r) nn("UICorner",{CornerRadius=UDim.new(0,r or 4),Parent=p}) end -- less smooth edges
...
local FULL=UDim2.new(0,870,0,548) -- 5% bigger
local Win=nn("Frame",{Size=UDim2.new(0,870,0,0),Position=UDim2.new(0.5,-435,0.5,0),BackgroundColor3=T.Window,BorderSizePixel=0,ClipsDescendants=true,Visible=false,Parent=Screen})
corner(Win,8) stroke(Win,Color3.fromRGB(32,32,32))

-- RAIL (15% smaller) + logo 10% bigger
local Rail=nn("Frame",{Size=UDim2.new(0,56,1,0),BackgroundColor3=T.Rail,BorderSizePixel=0,Parent=Win}) corner(Rail,8)
...
-- SETTINGS POPUP (transparent outline, shorter, moved left, covers gear)
...
Gear.MouseButton1Click:Connect(function() if settingsPop.Visible then closeSettings() else settingsPop.Visible=true outsideHit.Visible=true settingsPop.Size=UDim2.new(0,0,0,42) tw(settingsPop,0.22,{Size=UDim2.new(0,235,0,42)}) end end)
...
-- BOOK ANIMATION
local lastPos=UDim2.new(0.5,-435,0.5,-274) local menuOpen=false local menuAnimating=false
UIS.InputBegan:Connect(function(i,gp)
...
pcall(function() if queue_on_teleport then player.OnTeleport:Connect(function(ts) if ts==Enum.TeleportState.Started and state.autoReattach then pcall(function() queue_on_teleport([[task.wait(1.5) loadstring(game:HttpGet("https://raw.githubusercontent.com/Verticakos/UNIQ/refs/heads/main/UNIQ.lua"))()]]) end) end end) end end)
